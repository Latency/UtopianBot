...
public class Calculations extends MethodProvider {
   ...
   */
  public static double distanceBetween(final RSTile curr, final RSTile dest) {
  ...
  /**
   * Checks whether a point is within the rectangle that determines the bounds of game screen. This will work fine when in fixed mode. In resizable mode it will exclude any points
   * that are less than 253 pixels from the right of the screen or less than 169 pixels from the bottom of the screen, giving a rough area.
   * 
   * @param check The point to check.
   * @return <tt>true</tt> if the point is within the rectangle; otherwise <tt>false</tt>.
   */
  ...
  public Point worldToScreen(final int x, final int y, final int z) {
    ...
    if (_z >= render.zNear && _z <= render.zFar) {
      final int _x = (int) (render.xMultiplier * ((int) renderData.xOff + (int) (renderData.xX * x + renderData.xY * z + renderData.xZ * y)) / _z);
      final int _y = (int) (render.yMultiplier * ((int) renderData.yOff + (int) (renderData.yX * x + renderData.yY * z + renderData.yZ * y)) / _z);
