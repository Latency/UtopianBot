...
public class Prayer extends MethodProvider {
  ...
  public Prayer(final MethodContext ctx) {
    super(ctx);
  }
  ...
  public boolean setQuickPrayers(Book... prayers) {
    if (!isQuickPrayerOn())
      methods.interfaces.getComponent(INTERFACE_PRAYER_ORB, 1).interact(om.Server.toString("prayerQuickPrayers"));
    for (Book effect : prayers) {
      if (isQuickPrayerSet(effect))
        continue;
      methods.interfaces.getComponent(INTERFACE_PRAYER, 42).getComponent(effect.getComponentIndex()).interact(om.Server.toString("prayerSelect"));
      sleep(random(750, 1100));
    }
    return isQuickPrayerSet(prayers) && methods.interfaces.getComponent(INTERFACE_PRAYER, 42).getComponent(43).interact(om.Server.toString("prayerConfirmSelection"));
  }
  ...
  public int getPrayerPercentLeft() {
    return 100 * getPrayerLeft() / methods.skills.getCurrentLevel(Skill.PRAYER);
  }
  ...
  public boolean setPrayer(Book pray, boolean active) {
    if (isPrayerOn(pray) == active)
      return true;
    if (methods.skills.getRealLevel(Skill.PRAYER) < pray.getRequiredLevel())
      return false;
    ...
    if (methods.game.getTab() == Game.Tab.PRAYER) {
      ...
      if (component.isValid())
        component.interact(active ? om.Server.toString("prayerActivate") : om.Server.toString("prayerDeactivate"));
    }
  ...
  public boolean protectFrom(ProtectPrayer ppray) {
    ...
    if (methods.skills.getRealLevel(Skill.PRAYER) >= pray.getRequiredLevel() && !isPrayerOn(pray))
      setPrayer(pray, true);
