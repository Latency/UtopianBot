...
public class Combat extends MethodProvider {
  ...
  public int getHealth() {
    return getLifePoints() * 10 / methods.skills.getRealLevel(Skill.CONSTITUTION);
  }
  ...
  public int getWildernessLevel() {
    return methods.interfaces.get(381).getComponent(2).isValid() ? Integer.parseInt(methods.interfaces.get(381).getComponent(2).getText()
        .replace(om.Server.toString("combatLevel: "), "").trim()) : 0;
  }
   ...
   * 
   * @param fightMode The fight mode to set it to. From 0-3 corresponding to the 4 attacking modes; Else if there is only 3 attacking modes then, from 0-2 corresponding to the 3 attacking modes
