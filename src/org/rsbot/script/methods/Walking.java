...
public class Walking extends MethodProvider {
  ...
  public Walking(final MethodContext ctx) {
    super(ctx);
  }
   ...
   */
  public RSPath newTilePath(final RSTile[] tiles) {
    return new RSPath(methods, tiles);
  }
  ...
  public RSPath getPath(final RSTile destination) {
    return new RSPath(methods, destination);
  }
   ...
   * 
   * @param destination The destination tile.
   * @return <tt>true</tt> if the next tile was walked to; otherwise <tt>false</tt>.
   */
  ...
  public boolean walkTileOnScreen(final RSTile tileToWalk) {
    return methods.tiles.interact(methods.calc.getTileOnScreen(tileToWalk), om.Server.toString("walkingWalk")+ " ");
  }
  ...
  public boolean rest(final int stopEnergy) {
    ...
    for (int d = 0; d < 5; d++) {
      methods.interfaces.getComponent(INTERFACE_RUN_ORB, 1).interact(om.Server.toString("walkingRest"));
  ...
  public RSTile getDestination() {
    return (methods.client.getDestX() <= 0 ? null : new RSTile(methods.client.getDestX() + methods.client.getBaseX(), methods.client.getDestY() + methods.client.getBaseY()));
  }
  ...
  /**
   * Returns the collision map offset from the current region base on a given plane.
   * 
  ...
  /**
   * Waits until time has elapsed or player stops moving.
   * 
   * (Polls every 1/5 of a second)
   * 
   * @param timeout
   * @return
   */
  public final boolean waitToMove(final int timeout) {
    final Timer time = new Timer(timeout);
    while (time.isRunning() && methods.players.getMyPlayer().isMoving())
      sleep((time.getRemaining() > 200) ? 200 : (int) time.getRemaining());
