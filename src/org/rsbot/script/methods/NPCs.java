...
public class NPCs extends MethodProvider {
  ...
  public RSNPC[] getAll(final Filter<RSNPC> filter) {
    ...
    for (final int index : indices) {
      final Node node = Nodes.lookup(methods.client.getRSNPCNC(), index);
  ...
  /**
   * Returns the RSNPC that is nearest out of all of loaded RSNPCs accepted by the provided Filter.
   * 
   * @param filter
   *          Filters out unwanted matches.
   * @return An RSNPC object representing the nearest RSNPC accepted by the provided Filter; or null if there are no matching NPCs in the current region.
   */
  ...
  /**
   * Returns the RSNPC that is nearest out of all of the RSNPCs with the provided ID(s). Can return null.
   * 
   * @param ids
   *          Allowed NPC IDs.
   * @return An RSNPC object representing the nearest RSNPC with one of the provided IDs; or null if there are no matching NPCs in the current region.
   */
  ...
  /**
   * Returns the RSNPC that is nearest out of all of the RSNPCs with the provided name(s). Can return null.
   * 
   * @param names
   *          Allowed NPC names.
   * @return An RSNPC object representing the nearest RSNPC with one of the provided names; or null if there are no matching NPCs in the current region.
   */
