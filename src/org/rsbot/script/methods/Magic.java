...
public class Magic extends MethodProvider {
  ...
  public Magic(final MethodContext ctx) {
    super(ctx);
  }
  ...
  public boolean castSpell(final int spell) {
    if (methods.game.openTab(Game.Tab.MAGIC)) {
      ...
      if (inter != null) {
        ...
        return comp != null && comp.interact(om.Server.toString("magicCast"));
      }
    }
    return false;
  }
   ...
   * @return <tt>true</tt> if the "Auto-cast" interface option was clicked; otherwise <tt>false</tt>.
   */
  public boolean autoCastSpell(final int spell) {
    if (methods.settings.getSetting(43) != 4 && methods.game.openTab(Game.Tab.MAGIC)) {
      ...
      if (inter != null) {
        final RSComponent comp = inter.getComponent(spell);
        return comp != null && comp.interact(om.Server.toString("magicAutocast"));
      }
    }
    return false;
  }
  ...
  public boolean castSpellOn(final Object entity, final int spell) {
    ...
    if (castSpell(spell)) {
      if (entity instanceof RSCharacter) {
        return ((RSCharacter) entity).interact(om.Server.toString("magicCast"));
      } else if (entity instanceof RSObject) {
        return ((RSObject) entity).interact(om.Server.toString("magicCast"));
      } else if (entity instanceof RSGroundItem) {
        return ((RSGroundItem) entity).interact(om.Server.toString("magicCast"));
      }
    }
    return false;
  }
