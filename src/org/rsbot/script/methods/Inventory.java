...
public class Inventory extends MethodProvider {
  ...
  public Inventory(final MethodContext ctx) {
    super(ctx);
  }
   ...
   * 
   * @param leftClick <tt>true</tt> for left button click, <tt>false</tt> for right button.
   * @return <tt>true</tt> if item was selected, <tt>false</tt> if not.
   */
  public boolean clickSelectedItem(final boolean leftClick) {
    final RSItem item = getSelectedItem();
    return item != null && item.doClick(leftClick);
  }
   ...
   * @return <tt>true</tt> if your inventory contains an item with the ID provided; otherwise <tt>false</tt>.
  ...
  /**
   * Checks whether or not your inventory contains all of the provided item IDs.
   * 
   * @param itemID The item(s) you wish to evaluate.
   * @return <tt>true</tt> if your inventory contains at least one of all of the item IDs provided; otherwise <tt>false</tt>.
   * @see #containsOneOf(int...)
   */
  ...
  /**
   * Checks whether or not your inventory contains at least one of the provided item IDs.
   * 
   * @param itemID The item ID to check for.
   * @return <tt>true</tt> if inventory contains one of the specified items; otherwise <tt>false</tt>.
   * @see #containsAll(int...)
   */
   ...
   * @return <tt>true</tt> if the items were destroyed; otherwise <tt>false</tt>.
   */
  public boolean destroyItem(final int itemID) {
    RSItem item = getItem(itemID);
    if (item == null || !item.hasAction(om.Server.toString("inventoryDestroy"))) {
      return false;
    }
    while ((item = getItem(itemID)) != null) {
      if (methods.interfaces.get(94).isValid()) {
        ...
        item.interact(om.Server.toString("inventoryDestroy"));
      }
  ...
  public boolean dropAll(final int... ids) {
    ...
    for (final RSItem item : getItems(ids)) {
      isDropped = item.doClick(false);
      if (!isDropped)
        break;
    }
    return isDropped;
  ...
  /**
   * Drops all items except those with one of the provided IDs. This method drops items vertically going down the inventory.
   * 
   * @param ids The item IDs to drop.
   * @return <tt>true</tt> if items were dropped from the inventory; otherwise <tt>false</tt>.
   ...
   * @return <tt>true</tt> if we tried to drop the item, <tt>false</tt> if not (e.g., if item is undroppable)
   */
  public boolean dropItem(final int col, final int row) {
    ...
    if (methods.game.getTab() != Game.Tab.INVENTORY
      && !methods.interfaces.get(IBank.BANK.getID()).isValid()
    ...
    return item != null && item.getID() != -1 && item.interact(om.Server.toString("inventoryDrop"));
  }
   ...
   * 
   * @param includeStacks <tt>false</tt> if stacked items should be counted as a single item; otherwise <tt>true</tt>.
   * @return The count.
   */
  ...
  /**
   * Gets the count of all the items in the inventory without any of the provided IDs, ignoring stack sizes.
   * 
   ...
   * 
   * @return <tt>RSItem</tt> array of the current inventory items or an empty <tt>RSItem[]</tt> if unavailable.
   */
   ...
   * Gets all the items in the inventory.
   * 
   * @param cached If the inventory interface should be updated before returning the items.
   * @return <tt>RSItem</tt> array of the current inventory items or an empty <tt>RSItem[]</tt> if unavailable.
   */
   ...
   * 
   * @return The name of the current selected item, or null if none is selected.
   */
   ...
   * 
   * @return <tt>true</tt> if your inventory contains 28 items; otherwise <tt>false</tt>.
   */
  ...
  public boolean selectItem(final RSItem item) {
    ...
    if (!item.interact(om.Server.toString("inventoryUse"))) {
      return false;
    }
   ...
   * @return <tt>true</tt> if the first item has been "used" on the other; otherwise <tt>false</tt>.
   */
  ...
  public boolean useItem(final RSItem item, final RSItem targetItem) {
    methods.game.openTab(Game.Tab.INVENTORY);
    return selectItem(item) && targetItem.interact(om.Server.toString("inventoryUse"));
  }
  ...
  public boolean useItem(final RSItem item, final RSObject targetObject) {
    methods.game.openTab(Game.Tab.INVENTORY);
    return selectItem(item) && targetObject.interact(om.Server.toString("inventoryUse"), targetObject.getName());
  }
}
