...
public class Mouse extends MethodProvider {
  ...
  /**
   * Author - Enfilade Moves the mouse a random distance between 1 and maxDistance from the current position of the mouse by generating a random vector and then multiplying it by a
   * random number between 1 and maxDistance. The maximum distance is cut short if the mouse would go off screen in the direction it chose.
   * 
  ...
  /**
   * Author - Enfilade Moves the mouse a random distance between minDistance and maxDistance from the current position of the mouse by generating random vector and then multiplying
   * it by a random number between minDistance and maxDistance. The maximum distance is cut short if the mouse would go off screen in the direction it chose.
   * 
  ...
  public void moveRandomly(final int minDistance, final int maxDistance) {
    ...
    /*
     * If the maximum x is offscreen, subtract that distance/xvec from the maximum distance so the maximum distance will give a valid X coordinate
     */
    ...
    /*
     * If the maximum distance in the generated direction is too small, don't move the mouse at all
     */
    if (distance < minDistance) {
      return;
    }
    /*
     * With the calculated maximum distance, pick a random distance to move the mouse between maxDistance and the calculated maximum distance
     */
  ...
  /**
   * Moves the mouse to a given location with given randomness then clicks, then moves a random distance up to <code>afterOffset</code>.
   * 
   * @param x x coordinate
   * @param y y coordinate
   * @param randX x randomness (added to x)
   * @param randY y randomness (added to y)
   * @param leftClick <tt>true</tt> to left-click, <tt>false</tt>to right-click.
   * @param moveAfterDist The maximum distance in pixels to move on both axes shortly after moving to the destination.
   */
  ...
  /**
   * Moves the mouse to the specified point at a certain speed, then moves a random distance up to <code>afterOffset</code>.
   * 
   * @param speed The lower, the faster.
   * @param x The x destination.
   * @param y The y destination.
   ...
   * @param afterOffset The maximum distance in pixels to move on both axes shortly after moving to the destination.
   */
