...
public class Interfaces extends MethodProvider {
    ...
    for (final RSInterface iface : valid) {
      if (iface.getIndex() != 137) {
        ...
        for (int i = 0; i < len; i++) {
          ...
          if (child.containsText(om.Server.toString("iClickHereToContinue")) && child.isValid() && child.getAbsoluteX() > 10 && child.getAbsoluteY() > 300) {
            return child;
          }
        }
      }
   ...
   */
  public static boolean clickDialogueOption(final RSInterface inter, String option) {
   ...
   */
  public static boolean waitFor(final RSInterface iface, final boolean valid, final int timer) {
