...
public class Game extends MethodProvider {
  ...
  /**
   * Excludes: BankPin, TeleotherCloser, CloseAllInterface
   * 
  ...
  public boolean inRandom() {
    for (final Random random : methods.bot.getScriptHandler().getRandoms()) {
      if (random instanceof BankPins || random instanceof TeleotherCloser || random instanceof CloseAllInterface)
        continue;
   ...
   * 
   * @return <tt>true</tt> if the client is showing the login screen; otherwise <tt>false</tt>.
   */
   ...
   * 
   * @return <tt>true</tt> if the client is showing the welcome screen; otherwise <tt>false</tt>.
   */
   ...
   * 
   * @return <tt>true</tt> if random was run and the user has not chosen to disable LoginBot; otherwise <tt>false</tt>.
   */
  public boolean login() {
    if (methods.bot.disableAutoLogin)
      return false;

    for (final Random r : Collections.unmodifiableCollection(methods.bot.getScriptHandler().getRandoms())) {
      if (r instanceof LoginBot)
        return r.activateCondition();
    }
    return false;
  }
  ...
  public boolean logout(final boolean lobby) {
    ...
    if (methods.client.isSpellSelected() || methods.inventory.isItemSelected()) {
      ...
      Tab randomTab = getRandomTab();
      while (randomTab == currentTab)
        randomTab = getRandomTab();
  ...
  public int getCurrentWorld() {
    ...
    if (methods.game.isLoggedIn()) {
      ...
      world = Integer.parseInt(methods.interfaces.getComponent(550, 19).getText().replaceAll(om.Server.toString("gameFriendsList") + "<br>RuneScape ", ""));
