...
public class FriendChat extends MethodProvider {
  ...
  public FriendChat(final MethodContext ctx) {
    super(ctx);
  }
  ...
  public static class User implements Friend {
    ...
    public User(final OutputManager om, final String name, final RSComponent rank, final RSComponent world) {
      this.name = name;
      ...
      isInLobby = text.contains(om.Server.toString("fcLo"));
      if (!text.endsWith(".")) {
  ...
  /**
   * Joins the given channel. If we are already in a channel, it will leave it.
   * 
    ...
    if (isInChannel()) {
      ...
      if (list != null) {
        for (RSComponent c : list.getComponents()) {
          ...
          if (name != null && !name.isEmpty() && name.contains("..")) {
            ...
            if (actions != null) {
              for (String action : actions) {
                if (action != null) {
                  if (action.contains(om.Server.toString("fcAdd")) || action.contains(om.Server.toString("fcRemove"))) {
          ...
          users.add(new User(om, name, rank, world));
        }
  ...
  public String getName() {
    try {
      ...
      return name.substring(name.indexOf(om.Server.toString("fcTalkingIn: " + 12)));
  ...
  public String getOwner() {
    try {
      ...
      final String name = stripFormatting(methods.interfaces.getComponent(
          INTERFACE_FRIEND_CHAT, INTERFACE_FRIEND_CHAT_CHANNEL_INFO).getText());
      return name.substring(name.indexOf(om.Server.toString("fcOwner: " + 7)));
    } catch (final Exception e) {
      return null;
    }
  }
  ...
  public class FriendsList {
    ...
    public class Friend implements FriendChat.Friend {
      ...
      public Friend(String name, RSComponent world) {
        this.name = name;
        String text = world.getText();
        isOffline = text.contains(om.Server.toString("fcOf"));
        isInLobby = text.contains(om.Server.toString("fcLo"));
