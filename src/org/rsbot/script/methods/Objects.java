...
public class Objects extends MethodProvider {
  ...
  public Objects(final MethodContext ctx) {
    super(ctx);
  }
  ...
  /**
   * Returns all the <tt>RSObject</tt>s in the local region accepted by the provided Filter.
   * 
   * @param filter Filters out unwanted objects.
   * @return An <tt>RSObject[]</tt> of all the accepted objects in the loaded region.
   */
  ...
  /**
   * Returns all the <tt>RSObject</tt>s in the local region with the provided ID(s).
   * 
  ...
  /**
   * Returns all the <tt>RSObject</tt>s in the local region with the provided name(s).
   * 
  ...
  /**
   * Returns the <tt>RSObject</tt>s which are on the specified <tt>RSTile</tt> matching types specified by the flags in the provided mask.
   * 
   * @param t The tile on which to search.
  ...
  /**
   * Returns the <tt>RSObject</tt> that is nearest out of all objects that are accepted by the provided Filter.
   * 
   * @param filter Filters out unwanted objects.
   * @return An <tt>RSObject</tt> representing the nearest object that was accepted by the filter; or null if there are no matching objects in the current region.
   */
    ...
    for (int x = 0; x < 104; x++) {
      for (int y = 0; y < 104; y++) {
        ...
        for (final RSObject o : objs) {
          if (o != null && filter.accept(o)) {
            final double distTmp = Calculations.distanceBetween(methods.players.getMyPlayer().getLocation(), o.getLocation());
            if (cur == null) {
              dist = distTmp;
              cur = o;
  ...
  /**
   * Returns the <tt>RSObject</tt> that is nearest, out of all of the RSObjects with the provided ID(s).
   * 
   * @param ids The ID(s) of the RSObject that you are searching.
   * @return An <tt>RSObject</tt> representing the nearest object with one of the provided IDs; or null if there are no matching objects in the current region.
   */
  ...
  /**
   * Returns the <tt>RSObject</tt> that is nearest, out of all of the RSObjects with the provided name(s).
   * 
   * @param names The name(s) of the RSObject that you are searching.
   * @return An <tt>RSObject</tt> representing the nearest object with one of the provided names; or null if there are no matching objects in the current region.
   */
  ...
  /**
   * Returns the top <tt>RSObject</tt> on the specified tile matching types specified by the flags in the provided mask.
   * 
   * @param t The tile on which to search.
   * @param mask The type flags.
   * @return The top RSObject on the provided tile matching the specified flags; or null if none found.
   */
