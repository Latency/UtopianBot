...
public class RSObject extends MethodProvider {
  ...
  public RSObject(final MethodContext ctx, final org.rsbot.client.RSObject obj, final Type type, final int plane) {
    super(ctx);
    this.obj = obj;
    this.type = type;
    this.plane = plane;
   ...
   * @return returns true if clicked, false if object does not contain the desired action
   */
  ...
  public RSObjectDef getDef() {
    final org.rsbot.client.Node ref = Nodes.lookup(methods.client.getRSObjectDefLoader(), getID());
    if (ref != null) {
   ...
   * 
   * @return <tt>true</tt> if a path can be made to the specified object; otherwise <tt>false</tt>.
   */
