...
public abstract class RSCharacter extends MethodProvider {
  ...
  /**
   * Retrieves a reference to the client accessor. For internal use. The reference should be stored in a SoftReference by subclasses to allow for garbage collection when appropriate.
   * 
    ...
    if (isValid()) {
      ...
      try {
        ...
        for (int i = 0; i < 10; i++) {
          ...
          if (!methods.mouse.getLocation().equals(screenLoc) && methods.menu.doAction(action, option)) {
            return true;
          }
  ...
  public RSCharacter getInteracting() {
    ...
    if (interact < 32768) {
      final Node node = Nodes.lookup(methods.client.getRSNPCNC(), interact);
   ...
   * 
   * @return <tt>true</tt> if the character is dead/dying; otherwise <tt>false</tt>.
   */
  ...
  public String toString() {
    ...
    return "[anim=" + getAnimation() + (msg != null ? ",msg=" + getMessage() : "") + ",interact="
        + (inter == null ? "null" : inter.isValid() ? methods.players.getMyPlayer().getName() : om.Server.toString("rscInvalid")) + "]";
  }
}