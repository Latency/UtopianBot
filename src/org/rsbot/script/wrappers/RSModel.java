...
public abstract class RSModel extends MethodProvider {
   ...
   * 
   * @return All the points that are on the screen, if the model is not on the screen it will return null.
   */
   ...
   * 
   * @return First point that it finds on screen else a random point on screen of an object.
   */
  ...
  /**
   * Generates a rough central point. Performs the calculation by first generating a rough point, and then finding the point closest to the rough point that is actually on the RSModel.
   * 
  ...
  public Point getCentralPoint() {
    try {
      ...
      /*
       * Find a real point on the character that is closest to the central point
       */
      ...
      for (int index = 0; index < numVertices; index++) {
        if (points[index][2] == 1) {
          ...
          final double dist2 = Calculations.distanceBetween(central, point);
          if (dist2 < dist) {
   ...
   * 
   * @return <tt>true</tt> if the provided object is a model with the same points as this.
   */
  @Override
  ...
  /**
   * Draws a wireeframe of the model. It is optimized for fast rendering. Scripters should use this method instead of fetching every triangle and rendering them.
   * 
   ...
   * 
   * @return two dimensional array. The data format is posX = result[vertexIndex][0] posY = result[vertexIndex][1] visibleOnScreen = (result[vertexIndex][2] == 1);
   */
