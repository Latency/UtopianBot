...
public class FreakyForester extends Random implements MessageListener {
  public FreakyForester(MethodContext ctx) {
    super(ctx);
  }
  ...
  public boolean activateCondition() {
    ...
    forester = methods.npcs.getNearest(FORESTER_ID);
    if (forester != null) {
      sleep(random(2000, 3000));
      if (methods.npcs.getNearest(FORESTER_ID) != null) {
  ...
  public int getState() {
    if (done) {
      ...
      return 0;
    } else if (methods.inventory.containsOneOf(6178, 6179)) {
      return 0;
    } else if (phe.length > 0) {
      return 2;
    } else {
  ...
  public int loop() {
    ...
    } else if (methods.players.getMyPlayer().isMoving()) {
      return random(200, 500);
    }
    if (!done) {
      done = searchText(241, om.Server.toString("freakyThankYou")) || methods.interfaces.getComponent(242, 4).containsText(om.Server.toString("freakyLeave"));
    }
    /*
     * if (inventory.contains(6179)) { phe = new short[]{}; inventory.getItem(6179).doAction("Drop"); return random(500, 900); }
     */
    if (unequip && methods.inventory.getCount(false) != 28) {
      if (methods.game.getTab() != Game.Tab.EQUIPMENT) {
        methods.game.openTab(Game.Tab.EQUIPMENT);
        sleep(random(1000, 1500));
        methods.interfaces.get(Equipment.INTERFACE_EQUIPMENT).getComponent(17).doClick();
        return random(1000, 1500);
      }
      return random(100, 500);
    }
    if (methods.bank.isDepositOpen() || methods.inventory.getCount(false) == 28 && !methods.inventory.containsAll(6178)) {
      final int r = random(21, 27);
      if (methods.bank.isDepositOpen() && methods.bank.getBoxCount() == 28) {
        if (methods.interfaces.get(11).getComponent(17).getComponent(r).getComponentStackSize() > 1) {
          methods.interfaces.get(11).getComponent(17).getComponent(r).interact(om.Server.toString("bankDepositAll"));
        } else {
          methods.interfaces.get(11).getComponent(17).getComponent(r).interact(om.Server.toString("bankDeposit"));
        }
        return random(1000, 1500);
      } else if (methods.bank.isDepositOpen()) {
        methods.bank.close();
        return random(1000, 1500);
      }
      final RSObject box = methods.objects.getNearest(32931);
      if (!methods.calc.tileOnScreen(box.getLocation()) && methods.calc.distanceTo(methods.walking.getDestination()) < 8
          || methods.calc.distanceTo(methods.walking.getDestination()) > 40) {
        if (!methods.walking.walkTileMM(box.getLocation().randomize(3, 3))) {
          methods.walking.getPath(box.getLocation().randomize(3, 3)).traverse();
        }
        sleep(random(1200, 1400));
      }
      if (box.interact(om.Server.toString("bankDeposit"))) {
        return random(800, 1200);
      }
    }
    switch (getState()) {
      case 0: // Talk to forester
        if (methods.calc.tileOnScreen(forester.getLocation())
            && methods.calc.distanceTo(forester.getLocation()) <= 5) {
          forester.interact(om.Server.toString("slTalk"));
        } else if (methods.calc.distanceTo(forester.getLocation()) >= 5) {
          methods.walking.walkTileMM(methods.walking.getClosestTileOnMap(forester
              .getLocation().randomize(3, 3)));
          methods.camera.turnTo(forester.getLocation().randomize(3, 3));
        }
        return random(500, 800);
      case 1: // Talking
        // log("Talking"); //debug REMOVEME
        if (searchText(SEARCH_INTERFACE_ID, " " + om.Server.toString("freakyOne"))) {
          phe = Models.oneTail;
        } else if (searchText(SEARCH_INTERFACE_ID, " " + om.Server.toString("freakyTwo"))) {
          phe = Models.twoTail;
        } else if (searchText(SEARCH_INTERFACE_ID, " " + om.Server.toString("freakyThree"))) {
          phe = Models.threeTail;
        }
        if (searchText(SEARCH_INTERFACE_ID, " " + om.Server.toString("freakyFour"))) {
          phe = Models.fourTail;
        }
      ...
      case 2: // Kill pheasant
        ...
        final RSNPC pheasant = methods.npcs.getNearest(pheasantFilter);
        final RSGroundItem tile = methods.groundItems.getNearest(6178);
        if (tile != null) {
          methods.tiles.interact(tile.getLocation(), om.Server.toString("srTake"));
          return random(600, 900);
        } else if (pheasant != null) {
          // log("Pheasant ID = " + pheasant.getID());
          if (methods.calc.tileOnScreen(pheasant.getLocation()) && methods.calc.distanceTo(pheasant.getLocation()) <= 5) {
            pheasant.interact(om.Server.toString("sumAttack"));
            return random(1000, 1500);
          } else if (methods.calc.distanceTo(pheasant.getLocation()) >= 5) {
            methods.walking.walkTileMM(methods.walking.getClosestTileOnMap(pheasant.getLocation().randomize(3, 3)));
            methods.camera.turnTo(pheasant.getLocation().randomize(3, 3));
          }
        } else {
      ...
      case 3: // Get out
        if (!methods.calc.tileOnScreen(WALK_TO_TILE)) {
          if (methods.calc.tileOnMap(WALK_TO_TILE)) {
            methods.walking.walkTileMM(WALK_TO_TILE);
          } else {
            methods.walking.getPath(forester.getLocation().randomize(5, 5)).traverse();
          }
          return random(900, 1200);
        }
        final RSObject Portal = methods.objects.getNearest(PORTAL_ID);
        if (Portal == null) {
          log.info(om.OS.toString("freakyPortal"));
          return random(800, 1200);
        }
  ...
  public boolean searchText(final int interfac, final String text) {
    final RSInterface talkFace = methods.interfaces.get(interfac);
    if (!talkFace.isValid()) {
      return false;
    }
  ...
  public void messageReceived(final MessageEvent e) {
    ...
    if (serverString.contains(om.Server.toString("no ammo left"))) {
      unequip = true;
    }
  }
}