...
public class Prison extends Random {
  ...
  public Prison(MethodContext ctx) {
    super(ctx);
  }
  ...
  public boolean activateCondition() {
    if (methods.game.isLoggedIn()) {
      pete = methods.npcs.getNearest("Prison Pete");
      if (pete != null) {
        return methods.objects.getNearest(LEVER_ID) != null;
      }
    }
    return false;
  }
  ...
  @Override
  public int loop() {
    if (methods.npcs.getNearest("Prison Pete") == null) {
      return -1;
    }
    if (!talkedtopete) {
      methods.camera.setPitch(true);
      if (methods.camera.getAngle() < 175 || methods.camera.getAngle() > 185) {
        methods.camera.setAngle(random(175, 185));
    ...
    switch (state) {
      case 0:
        ...
        if (methods.inventory.getCount(false) == 28 && !methods.inventory.containsAll(DOOR_KEY)) {
          log("Not enough space for this random. Depositing an Item");
          final RSObject depo = methods.objects.getNearest(32924);
          if (depo != null) {
            if (!methods.calc.tileOnScreen(depo.getLocation())) {
              if (!methods.walking.walkTileMM(depo.getLocation().randomize(3, 3))) {
                methods.walking.getPath(depo.getLocation().randomize(3, 3)).traverse();
                return random(500, 700);
              }
              return random(1000, 1500);
            }
            methods.camera.turnTo(depo, 20);
            if (depo.interact("Deposit")) {
              ...
              if (methods.interfaces.get(IBank.DEPOSIT_BOX.getID()).isValid()) {
                ...
                methods.interfaces.get(11).getComponent(17).getComponent(random(16, 17)).interact("Dep");
                sleep(random(700, 1200));
        ...
        if (methods.interfaces.get(228).isValid()
            && methods.interfaces.get(228).containsText("How do")) {
          methods.interfaces.get(228).getComponent(3).doClick();
          return random(500, 600);
        }
        if (methods.interfaces.canContinue()) {
          methods.interfaces.clickContinue();
        ...
        if (!talkedtopete && pete != null
            && !methods.interfaces.get(228).isValid()
            && !methods.interfaces.canContinue()) {
          if (!methods.calc.tileOnScreen(pete.getLocation())) {
            methods.walking.walkTileMM(pete.getLocation());
            return random(1000, 1400);
          }
          if (pete.interact("talk")) {
            return random(1500, 1600);
          }
          methods.camera.turnTo(pete.getLocation());
          return random(500, 600);
        }
        if (unlocked == 3) {
          state = 4;
      ...
      case 1:
        ...
        if (methods.interfaces.get(273).getComponent(3).isValid()) {
          if (atLever()) {
            if (balloonToPop != null && methods.interfaces.get(273).getComponent(4).interact("Close")) {
              state = 2;
        ...
        final RSObject lever = methods.objects.getNearest(LEVER_ID);
        if (lever != null && talkedtopete) {
          if (!methods.calc.tileOnScreen(lever.getLocation())) {
            methods.walking.walkTileMM(lever.getLocation());
            return random(1000, 1200);
          }
          if (!methods.players.getMyPlayer().isMoving()
              && methods.calc.tileOnScreen(lever.getLocation())) {
            ...
            methods.camera.turnTo(lever.getLocation());
            return random(500, 600);
          }
        }
        if (!talkedtopete) {
      ...
      case 2:
        ...
        if (!methods.inventory.containsAll(DOOR_KEY)) {
          if (methods.calc.tileOnScreen(balloonToPop.getLocation())) {
            balloonToPop.interact("Pop");
            return random(1200, 1400);
          }
          if (!methods.players.getMyPlayer().isMoving()) {
            methods.walking.walkTileMM(balloonToPop.getLocation().randomize(2, 2));
            return random(500, 750);
          }
          return random(500, 750);
        }
      ...
      case 3:
        ...
        if (pete != null && !methods.calc.tileOnScreen(pete.getLocation()) && !methods.interfaces.get(243).isValid()) {
          methods.walking.walkTileMM(pete.getLocation());
          return random(400, 600);
        }
        if (!methods.inventory.containsAll(DOOR_KEY) && methods.npcs.getNearest(PRISON_MATE) != null && unlocked <= 2 && key) {
          unlocked++;
          state = 0;
          balloonToPop = null;
          return random(350, 400);
        ...
        if (methods.inventory.containsAll(DOOR_KEY) && !methods.players.getMyPlayer().isMoving()) {
          methods.inventory.getItem(DOOR_KEY).interact("Return");
      ...
      case 4:
        ...
        if (!methods.calc.tileOnScreen(doorTile)) {
          methods.walking.walkTileMM(doorTile);
          return random(400, 500);
        }
        if (methods.calc.tileOnScreen(doorTile)) {
          final RSObject gate = methods.objects.getNearest(11177, 11178);
          if (gate != null) {
            gate.interact("Open");
          }
  ...
  public static short[] setItemIDs(final int b2p) {
  ...
  public boolean interfaceContains(final String s) {
    final RSInterface[] all = methods.interfaces.getAll();
  ...
  public boolean atLever() {
    if (methods.interfaces.get(273).getComponent(3).isValid()) {
      final Filter<RSModel> filter = RSModel.newVertexFilter(setItemIDs(methods.interfaces.get(273).getComponent(3).getModelID()));
      balloonToPop = methods.npcs.getNearest(new Filter<RSNPC>() {
        @Override
