...
public class LostAndFound extends Random {
  public LostAndFound(MethodContext ctx) {
    super(ctx);
  }
  ...
  public boolean activateCondition() {
    return methods.game.isLoggedIn() && methods.objects.getNearest(appendN) != null;
  }
  ...
  private int getOddAppendage() {
    final int[] settings = this.methods.settings.getSettingArray();
    try {
  ...
  public int loop() {
    ...
    if (methods.objects.getNearest(appendN) == null) {
      return -1;
    }
    ...
    try {
      ...
      if (!methods.calc.tileOnScreen(tile)) {
        methods.walking.getPath(tile).traverse();
        sleep(random(700, 900));
        while (methods.players.getMyPlayer().isMoving()) {
          sleep(100);
        }
      }
      if (obj.interact(om.Server.toString("lostOperate"))) {
