...
public class MouseHandler {
  ...
  /**
   * Applies a midpoint algorithm to the Vector of points to ensure pixel to pixel movement
   * 
   * @param points The vector of points to be manipulated
   */
  public static void adaptiveMidpoints(final java.util.Vector<Point> points) {
    int i = 0;
   ...
   * @param msForMove The ammount of time taken to traverse the spline. should be a value from {@link #fittsLaw}
  ...
  /**
   * Returns an array of gaussian values that add up to 1 for the number of steps Solves the problem of having using an intergral to distribute values
   * 
  ...
  public static Point[] generateControls(final int sx, final int sy, final int ex, final int ey, int ctrlSpacing, int ctrlVariance) {
    ...
    for (int i = 1; i < ctrlPoints + 1; i++) {
      ...
      /**
       * Hopefully {@link java.util.Random} is thread safe. (it is in Sun JVM 1.5+)
       */
  ...
  /**
   * Generates a spline that moves no more then one pixel at a time TIP: For most movements, this spline is not good, use <code>applyDynamism</code>
   * 
  ...
  public MouseHandler(final InputManager inputManager) {
   ...
   * 
   * @param speed the speed to move the mouse. Anything under {@link #DEFAULT_MOUSE_SPEED} is faster than normal.
   * @param x1 from x
   * @param y1 from y
   * @param x2 to x
   * @param y2 to y
  ...
  public void moveMouse(final int speed, final int x1, final int y1, final int x2, final int y2, int randX, int randY) {
    ...
    try {
      ...
      moveMouse(speed, x1, y1, x2, y2, spline);
    } catch (final Exception ignored) {
    }
  }

  /**
   * Moves the mouse from a position to another position with a pre-defined spline.
   * 
   * @param speed the speed to move the mouse. Anything under {@link #DEFAULT_MOUSE_SPEED} is faster than normal.
   * @param x1 from x
   * @param y1 from y
   ...
   */
  public void moveMouse(final int speed, final int x1, final int y1, final int x2, final int y2, final Point[] spline) {
    if (x2 == -1 || y2 == -1 || x1 == -1 || y1 == -1) {
      return;
    }
    try {
