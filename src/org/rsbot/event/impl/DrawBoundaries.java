...
public class DrawBoundaries implements PaintListener {
  ...
  public final void onRepaint(final Graphics render) {
    ...
    final int blocks[][] = methods.client.getRSGroundDataArray()[methods.client.getPlane()].getBlocks();
    final int baseX = methods.client.getBaseX();
    final int baseY = methods.client.getBaseY();
    for (int i = 0; i < screenPoints.length; i++) {
      for (int j = 0; j < screenPoints[i].length; j++) {
        final int x = i + baseX - 1;
        final int y = j + baseY - 1;
        Point mini = methods.calc.worldToMinimap(x - 0.5, y - 0.5);
        if (mini.x == -1 || mini.y == -1) {
          mini = null;
        }
        minimapPoints[i][j] = mini;
        Point screen = methods.calc.tileToScreen(new RSTile(x, y), 0, 0, 0);
    ...
    for (int i = 1; i < 104; i++) {
      for (int j = 1; j < 104; j++) {
        ...
        // 0.5,0.5, 0);
        /*
         * if (cent.x != -1 && cent.y != -1) { render.setColor(Color.yellow); render.drawString("" + Calculations.getRealDistanceTo(cur.getX()-baseX, cur.getY()-baseY, i, j, false), (int)cent.getX(), (int)cent.getY()); }
         */
      }
    }
    final Point mini = methods.players.getMyPlayer().getMinimapLocation();
    render.setColor(Color.red);
