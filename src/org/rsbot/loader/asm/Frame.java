...
final class Frame {
  ...
  /**
   * Flag used for LOCAL and STACK types. Indicates that if this type happens to be a long or double type (during the computations of input frames), then it must be set to TOP
   * because the second word of this value has been reused to store other data in the basic block. Hence the first word no longer stores a valid long or double value.
   */
  ...
  /**
   * Base kind of the uninitialized base types. The BASE_VALUE of such types in an index into the type table (the Item at that index contains both an instruction offset and an
   * internal class name).
   */
  ...
  /**
   * The stack size variation corresponding to each JVM instruction. This stack variation is equal to the size of the values produced by an instruction, minus the size of the
   * values consumed by this instruction.
   */
   ...
   * <p/>
   * When the stack map frames are completely computed, this field is the actual number of types in {@link #outputStack}.
   */
  ...
  /**
   * The types that are initialized in the basic block. A constructor invocation on an UNINITIALIZED or UNINITIALIZED_THIS type must replace <i>every occurence</i> of this type in
   * the local variables and in the operand stack. This cannot be done during the first phase of the algorithm since, during this phase, the local variables and the operand stack
   * are not completely computed. It is therefore necessary to store the types on which constructors are invoked in the basic block, in order to do this replacement during the
   * second phase of the algorithm, where the frames are fully computed. Note that this array can contain types that are relative to input locals or to the input stack (see below
  ...
  private int get(final int local) {
    ...
    // this local has never been assigned in this basic block, so it is still equal to its value in the input frame

    if (outputLocals == null || local >= outputLocals.length)
      return LOCAL | local;

    return (type == 0 ? (outputLocals[local] = LOCAL | local) : type);
  }
   ...
   * @param desc the descriptor of the type to be pushed. Can also be a method descriptor (in this case this method pushes its return type onto the output frame stack).
   */
   ...
   * 
   * @param desc the descriptor of the type to be popped. Can also be a method descriptor (in this case this method pops the types corresponding to the method arguments).
   */
   ...
   * @param cw the ClassWriter to which this label belongs.
   * @param t a type
   * @return t or, if t is one of the types on which a constructor is invoked in the basic block, the type corresponding to this constructor.
   */
  ...
  /**
   * Merges the input frame of the given basic block with the input and output frames of this basic block. Returns <tt>true</tt> if the input frame of the given label has been
